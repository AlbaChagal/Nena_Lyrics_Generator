...
class WordsInTextCounter:
    ...
    @staticmethod
    def get_words_from_logits(logits: torch.Tensor, idx2word: Dict[int, str], is_debug: bool = False) -> List[str]:
        predicted_chars: torch.Tensor = logits.argmax(dim=-1)  # shape: (batch, seq_len)
        generated_text: str = "".join([idx2word[idx] for idx in predicted_chars[0].tolist()])
    ...
    def count_valid_words(self, logits: torch.Tensor, idx2word: Dict[int, str], is_debug: bool = False) -> float:
